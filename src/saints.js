// Éphéméride : prénom traditionnellement fêté chaque jour du calendrier
// civil français. Donnée statique (aucun appel réseau, aucune dépendance
// HA) — compilée à partir d'un calendrier public et volontairement
// raccourcie (un seul prénom, sans qualificatif) pour tenir dans une
// petite zone d'affichage. Une poignée de jours sans prénom propre
// (fêtes religieuses sans prénom associé, 29 février) n'a pas d'entrée :
// `saintOfDay` renvoie alors `null` et l'appelant n'affiche rien ce
// jour-là plutôt que d'inventer une valeur.
const RAW = `
01-01:Marie
01-02:Odile
01-03:Geneviève
...
export function saintOfDay(date) {
  const mm = String(date.getMonth() + 1).padStart(2, "0");
  const dd = String(date.getDate()).padStart(2, "0");
  return SAINTS.get(`${mm}-${dd}`) || null;
}
